    ...
    private void initCharts() {
        ...
        try {
            ...
            for (String[] strings : usersList) {
                ...
                areaSeries.getData().add(new XYChart.Data<>(currentDate.getMonth().getDisplayName(TextStyle.FULL, Locale.getDefault()), Integer.parseInt(strings[1])));
            }
            ...
            setPercentageIncrease(Double.parseDouble(userNum), Double.parseDouble(usersList.get(usersList.size() - 2)[1]), userGrow);
            ...
            setPercentageIncrease(Double.parseDouble(bookNum), Double.parseDouble(bBooks.get(bBooks.size() - 2)[3]), booksGrow);

            String bBookNum = dataAccessObject.countRecords(User.class, "select count(*) from borrowed_books where library_id=" + libraryID + " and is_returned=0");
            borrowedAmount.setText(bBookNum);
            setPercentageIncrease(Double.parseDouble(bBookNum), Double.parseDouble(bBooks.get(bBooks.size() - 2)[1]), borrowedGrow);
